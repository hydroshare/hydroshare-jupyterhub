...
class iCommands(object):
	def __init__(self, hydroshare):
		...
		if 'IRODS_PLUGINS_HOME' not in os.environ.keys():
			...
			os.environ['IRODS_PLUGINS_HOME'] = '%s/icommands/plugins/' % self.irods_dir
			os.environ['IRODS_ENVIRONMENT_FILE'] = '%s/.irods/irods_environment.json' % self.irods_dir
			os.environ['IRODS_AUTHENTICATION_FILE'] = '%s/.irods/.irodsA' % self.irods_dir

		chk = Popen(['ienv'], stdout=PIPE, stderr=STDOUT, shell=True)
		stdout = chk.communicate() 
		chk_returncode = chk.returncode
		if chk_returncode != 0:
			print('iCommands were not found on the system, preparing to install.')
			# need to install icommands

			print('Downloading iCommands...', end='', flush=True)
			if not os.path.exists('/home/jovyan/libs/icommands.sh'):
					get_cmd = 'wget --no-http-keep-alive -O /home/jovyan/libs/icommands.sh https://pods.iplantcollaborative.org/wiki/download/attachments/28117338/irods-icommands-4.1.9-ubuntu-12.installer?version=1&modificationDate=1473720729000&api=v2'
					download_icommands = Popen([get_cmd], stdout=PIPE, stderr=STDOUT, shell=True)
					t = download_icommands.communicate()
			print('done', flush=True)

			print('Installing iCommands...', end='', flush=True)
			install_cmd = 'chmod +x /home/jovyan/libs/icommands.sh; echo "%s" | /home/jovyan/libs/icommands.sh ' % self.irods_dir
			installicommands = Popen(install_cmd, stdout=PIPE, stderr=STDOUT, shell=True)
			stdout = installicommands.communicate() 
	...
	def ils(self):
		...
		if cmd.returncode != 0:
			...
			return []
		return [s.replace('C-','').strip() for s in stdout.split('\n')[1:] if s != '']
	...
	def iget(self, filename, destination=None, unzip=True):
		...
		if unzip and filename[-3:] == 'zip':
			...
			if os.path.exists(zdestination):
				res = input('This directory already exists, do you want to overwrite it [Y/n]?')
	...
	def irsync(self, source, target, source_irods=False, target_irods=True, dryrun=False):
		"""sync files between Jupyter and iRODs 
		
		args:
		-- source: name of the source sync file
		-- target: name of the target sync file
		-- source_irods: specifies if the source exists in HydroShare iRODs (boolean), default False
		-- target_irods: specifies if the target exists in HydroShare iRODs (boolean), default True
		...
		if dryrun:
			liverun = False
			cmd = Popen(['%s -l' % irods_cmd], stdout=PIPE, stderr=STDOUT, shell=True)
			stdout = cmd.communicate()[0].decode('ascii')
			if cmd.returncode != 0:
				print('Failed during syncronization dry run: %s' % stdout, flush=True)
				return False
			print(stdout, flush=True)
			res = input('Would you like to continue with syncronization [Y/n]?')
